...
"""for i in range(20):
    ...
    for l in range(6):
        ...
        for j in range(N_tacaka):
            y1.append(izlazni(d,V,T1[i],av1))
            V+=dV
        Fluks+=E_W(x1,y1)
        d+=dl
    Fu.append(Fluks)
...
av=A/Dopl_sirina(T) 
y5=izlazni(d,x,T,av) 
plt.plot(xla,y5,label='d=5Rk/6')
d+=Rk/6 
plt.title('Profili za različite preseke kome na 207 K ili 1.09 AU ')
plt.yscale('log')
plt.legend(loc='best')
plt.show() 
#plt.xlim(0,0.2e+7)
