...
#    q = ((S*(1-np.exp(-tau)))/(rastojanje(T)**2))*(Rk-d)*dl*2*np.pi
    q = ((S*(1-np.exp(-tau)))/(rastojanje(T)**2))
    return q
...
plt.show()"""
fig1=plt.figure(num = 1, figsize=(11,9), dpi=150)
#plt.suptitle('Zavisnost ukupne koncentracije Na od rastojanja od jezgra za različite temperature', fontsize = 18)
d=0
T=200
poluprecnik=[]
kralj=[]
...
y0=izlazni(d,x,T,av)
#plt.plot(poluprecnik,kralj,label='1.69 AU, 200K', color = 'blue', lw=2) 
#plt.xlim(0,0.2e+7) 
T=207
poluprecnik=[]
kralj=[] 
...
av=A/Dopl_sirina(T) 
y1=izlazni(d,x,T,av)
#plt.plot(poluprecnik,kralj,label='1.09 AU, 207K', color='green', lw=2) 
#plt.xlim(0,0.2e+7) 
T=220 
poluprecnik=[]
kralj=[] 
...
av=A/Dopl_sirina(T) 
y2=izlazni(d,x,T,av) 
#plt.plot(poluprecnik,kralj,label='0.49 AU, 220K', color='red', lw=2) 
#plt.xlim(0,0.2e+7) 
T=230
poluprecnik=[]
kralj=[] 
...
av=A/Dopl_sirina(T) 
y3=izlazni(d,x,T,av) 
#plt.plot(poluprecnik,kralj,label='0.27 AU, 230K', color='cyan', lw=2) 
#plt.xlim(0,0.2e+7) 
T=250 
poluprecnik=[]
kralj=[] 
...
av=A/Dopl_sirina(T) 
y4=izlazni(d,x,T,av) 
#plt.plot(poluprecnik,kralj,label='0.10 AU, 250K', color='purple', lw = 2) 
#plt.xlim(0,0.2e+7) 
T=260 
poluprecnik=[]
kralj=[] 
...
av=A/Dopl_sirina(T) 
y5=izlazni(d,x,T,av) 
#plt.plot(poluprecnik,kralj,label='0.06 AU, 260K', color='orange', lw = 2) 
#plt.xlim(0,0.2e+7)
mp.rcParams.update({'font.size':18})
#plt.legend(loc='best')
"""plt.xlabel('Udaljenost od kome [m]')
plt.ylabel('Koncentracija Na [m^-3]')
plt.yscale('log')"""
#plt.xticks(np.linspace(min(x), max(x), num = 5))
"""print(lp(200)+ld(200))
...
#    x[i]=milance[len(x)-i]
plt.suptitle('Zavisnost intenziteta linije Na od talasne dužine')
plt.plot(x,y0,label='1.69 AU, 200K', lw = 2)
plt.plot(x,y1,label='1.09 AU, 207K', lw = 2) 
plt.plot(x,y2,label='0.49 AU, 220K', lw = 2) 
plt.plot(x,y3,label='0.27 AU, 230K', lw = 2) 
plt.plot(x,y4,label='0.10 AU, 250K', lw = 2) 
plt.plot(x,y5,label='0.06 AU, 260K', lw = 2) 
plt.legend(loc='best')
plt.ylabel('Intenzitet')
plt.xlabel('Talasna dužina [nm]')
plt.xlim(588.9e-9,589.1e-9)
#plt.xlim(1.5*(1e10+5.0896e14),4.1*(1e10+5.0896e14))
plt.ylim(0.00000001,10000)
#plt.show()
#plt.savefig('Profili(T)1')
