...
def opt_dub(d,V,T,av): #opticka dubina za nehomogenu komu
    r1=Rk
    suma_opt=0
    broj=br_ljuspica - 1 - math.floor((d-Rn)/dr)
    for i in range(broj):
        r2=r1-dr
...
def poc_intez(L,T): #pocetni intezitet preko plankove fje
    plank=(2*h*c*c)/(L**5*(np.exp((h*c)/(L*k*T))-1))
    return plank

def poc_intez_sunce(V,T): #pocetni intezitet preko plankove fje
    plank=(2*h*V**3)/(c**2*(np.exp((h*V)/(k*T))-1))
    return plank
...
def fja_izv(dL,av):
    ...
    suma=0
    for i in range(100):
        suma+=(poc_intez(L,T)*Fojtov_profil(c/L,av))*dL
        L+=dL
    suma=suma*1/2
    return suma 

def izlazni(V,av):
    izla=fja_izv(dL,av)*(1-np.exp(-opt_dub(d,V,T,av)))
    return izla 
...
V=V2
for i in range(100):
    y1.append(izlazni(V,av1)/poc_intez_sunce(V,Ts))
   ...
   # y3.append(izlazni(V,av3)/poc_intez(V,Ts))
    ...
    print(izlazni(V,av1), "/", poc_intez_sunce(V,Ts), "=", izlazni(V,av1)/poc_intez_sunce(V,Ts))
    x.append(c/V)     
    V-=dV
