...
def koef_aps(V,T,r,av,raspodela): #koeficijent apsorpcije
    B = ajnstajnB(A)
    if (raspodela==0):
        konc_aps = n0 = kon_Na_koma(T,r) #MAXBOLC statistika    
    else:
        konc_aps = n0 = kon_Na_koma(T,r) #Bolcmanova raspodela
...
def opt_dub(d,V,T,av,raspodela): #opticka dubina za nehomogenu komu
    r1 = Rk
    suma_opt = 0
    ...
    for i in range(broj):
        ...
        suma_opt += koef_aps(V,T,r1,av,raspodela)*ds
        r1 = r2
...
def izlazni(V,av,raspodela): #izlazni intenzitet normiran tako da je I0=1, a funkcija izvora S=1/2
    tau = opt_dub(d,V,T,av,raspodela) #opticka dubina
...
plt.plot(x,y0,label='1.69 AU, 200K') 
plt.plot(x,y1,label='1.09 AU, 207K') 
