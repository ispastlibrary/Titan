...
def temp(D):
    albedo=0
    L=3.846e+26 #luminoznost sunca W
    sigma=5.670373e-8 #SB konstanta
    t=math.pow(L*(1-albedo)/(16*np.pi*sigma*D*D),1/4)    
    return t
...
av4 = (A+vd_vals(nh)+stark(Nswe,Tswe))/Dopl_sirina(T4)
#av2 = A/Dopl_sirina(T)
...
for i in range(N_tacaka):
    ...
    q.append(izlazni(V,av3))
    g.append(izlazni(V,av4))
    x.append(c/V)
    V += dV
x.reverse()
y.reverse()
