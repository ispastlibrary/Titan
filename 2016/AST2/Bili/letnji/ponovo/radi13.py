...
kralj=[]    
poluprecnik=[]
#T1=np.linspace(100,250,20)
T1=[220]
u1=[]
EW1=[]
o1=np.linspace(c/V2,c/V1,N_tacaka)
for i in range(1):
    av1=A/Dopl_sirina(T1[i])
    V=V1
    for j in range(N_tacaka):
        u1.append(izlazni(V,T1[i],av1))
        V+=dV
    EW1.append(E_W(o1,u1))
    u1=[]

"""produkcija=Q_Na(T)
rastojanjee=rastojanje(T)"""
plt.plot(T1,EW1)
plt.suptitle('Zavisnost EW od temperature komete')
