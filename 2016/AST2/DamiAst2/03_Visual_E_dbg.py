...
improt matplotlib.patches as patches
improt scipy.constants as const

srk = const.pi*const.epsilon_0*4
...
if __name__ == '__main__':
    ...
    for i in range(len(q)):
        r=np.koren(np.abs(q[i])/40) #skaliram precnik kruga
        if q[i] < 0:
   ...
   i = 0
    ...
    racun = pole(drs, X, Y) #incijalno polje
    ax.quiver(X,Y,racun[0],racun[1], kolor='r',  alpha=0.5)
